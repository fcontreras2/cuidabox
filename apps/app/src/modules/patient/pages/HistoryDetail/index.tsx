...
export default function EventDetail({ id }: Props) {
  const t = useTranslations("modules-patient-pages-HistoryDetail");
  const params = useParams();
  const patientId = typeof params?.id === "string" ? params.id : "";
  const result = useHistoryDetail(id);
    ...
    <PhoneFrame>
      ...
      >
        ...
        <div className="relative flex items-center justify-between">
          <BackButton
            variant="dark"
            fallbackHref={`/patient/${patientId}/history`}
          />
            ...
            <div className="flex flex-col gap-2">
              {related.map((r) => {
                ...
                return (
                  <Link
                    key={r.id}
                    href={`/patient/${patientId}/history/${r.id}`}
